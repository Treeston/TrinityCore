...
static std::string pretty(uint32 t)
{
    std::string s;
    uint32 l = 1000;
    while (l <= t)
        l *= 1000;
    l /= 1000;
    s.append(std::to_string(t/l));
    while (l > 1)
    {
        t %= l;
        l /= 1000;
        s.append(Trinity::StringFormat(",%03d", t/l));
    }
    return s;
}

static std::string pretty(float f)
{
    std::string s = pretty(uint32(f + 0.005f));
    s.append(Trinity::StringFormat("%.2f", f - std::floor(f)), 1);
    return s;
}
...
{
    public:
    npc_damage_test_dummy() : CreatureScript("npc_damage_test_dummy") {}

    struct npc_damage_test_dummyAI : public NullCreatureAI
    {
        enum Modes { MODE_NULL, MODE_DPS };
        npc_damage_test_dummyAI(Creature* creature) : NullCreatureAI(creature), _mode(MODE_NULL), _attemptCountdown(0), _attemptTimer(0), _attemptDuration(0), _healthUpdateInterval(1000), _rageGainInterval(3000) { }
        ...
        friend class npc_damage_test_controller;
    };

    CreatureAI* GetAI(Creature* creature) const override
    {
        return new npc_damage_test_dummyAI(creature);
    }
};

class npc_damage_test_controller : public CreatureScript
{
    public:
    npc_damage_test_controller() : CreatureScript("npc_damage_test_controller") {}

    struct npc_damage_test_controllerAI : public NullCreatureAI
    {
        enum GossipOffsets
        {
        ...
        struct BuffInfo { uint32 spellId; uint32 castDummy; BuffInfo(uint32 id, uint32 dummy) : spellId(id), castDummy(dummy) { } BuffInfo(uint32 id) : spellId(id), castDummy(0) {} };
        std::vector<BuffInfo> const buffs =
        {
        ...
        std::vector<BuffInfo> const debuffs =
        {
             8647, // expose armor
              770, // faerie fire
            48564, // mangle
        ...
        static bool yes(Aura const* /*a*/) { return true; }
        typedef npc_damage_test_dummy::npc_damage_test_dummyAI FriendAI;
        FriendAI* GetFriendAI()
        {
        ...
        static bool UnavailableToTeam(TeamId team, uint32 spellId)
        {
            switch (spellId)
            {
        ...
        static const int8 NUM_DUMMY = 3;
        ObjectGuid dummy[NUM_DUMMY];
        ObjectGuid casterguid(BuffInfo info) { return (info.castDummy && info.castDummy <= NUM_DUMMY) ? dummy[info.castDummy - 1] : me->GetGUID(); }
        bool iscasterguid(ObjectGuid const& guid) { if (guid == me->GetGUID()) return true; for (int8 i = 0; i < NUM_DUMMY; ++i) if (guid == dummy[i]) return true; return false; }
        npc_damage_test_controllerAI(Creature* creature) : NullCreatureAI(creature) { }
        void InitializeAI() override
        {
            bool isDummy = false;
            ...
            for (auto const& pair : me->GetOwnedAuras())
                if (SpellInfo const* ourInfo = pair.second->GetSpellInfo())
                    for (int8 i = 0; i < MAX_SPELL_EFFECTS; ++i)
                        ...
                        {
                            if (!(
                                (effect->GetMiscValue() == SPELLMOD_ALL_EFFECTS) ||
        ...
        void AddBuff(Unit* unit, BuffInfo info)
        {
            Unit* caster = me;
            ...
            buff->SetMaxDuration(5 * HOUR * IN_MILLISECONDS); // just some convenience for buff sorting addons
            buff->SetDuration(17 * IN_MILLISECONDS);
            for (Unit* minion : unit->m_Controlled)
                AddBuff(minion, info);
        }
        bool HasAnyBuff(Unit* target)
        {
        ...
        void AddDebuff(FriendAI* ai, BuffInfo info)
        {
            Unit* caster = me;
            if (info.castDummy && info.castDummy <= NUM_DUMMY)
                caster = ObjectAccessor::GetUnit(*me, dummy[info.castDummy - 1]);
            ...
            switch (action - GOSSIP_ACTION_INFO_DEF)
            {
                ...
                case GOSSIP_OFFSET_ATTEMPT_DPS_30SEC:
                    if (current)
                        break;
                    CloseGossipMenuFor(player);
                    dummyAI->BeginAttempt(player, FriendAI::MODE_DPS, Seconds(30));
                    break;
                case GOSSIP_OFFSET_ATTEMPT_DPS_150SEC:
                    if (current)
                        break;
                    CloseGossipMenuFor(player);
                    dummyAI->BeginAttempt(player, FriendAI::MODE_DPS, Seconds(150));
                    break;
                case GOSSIP_OFFSET_ATTEMPT_DPS_360SEC:
                    if (current)
                        break;
                    CloseGossipMenuFor(player);
                    dummyAI->BeginAttempt(player, FriendAI::MODE_DPS, Minutes(6));
                    break;
...
{
    new npc_damage_test_dummy();
    new npc_damage_test_controller();
}
